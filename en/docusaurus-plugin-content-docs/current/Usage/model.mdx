...
:::usage
**`/calc [statistic] [sign] [expression] (*character) (comment)`**
:::

The commands require you to choose the statistic to use, but also a sign and an expression. The valid signs are: `+`, `-`, `*`, `/`, `^` (power), `%` (modulo), `/` (division), `≠` (not equal to), `=` (equal to), `⩾` (greater than or equal to), `⩽` (less than or equal to), `>` (greater than), `<` (less than).
