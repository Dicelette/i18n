...
- `$`: Symbolizes the value of the statistic used.
- The name of a statistic (which will be replaced by its value).
- Dice combinations (such as `1d6`).
- `{exp}`: This syntax allows you to insert an expression directly into the dice, using the calculated part of the command. You can specify a default value using the `{exp||default}` syntax (like `{exp||2}`). Otherwise, the default value will be `1`.
