...
The bot uses the [dice roller](https://dice-roller.github.io/documentation/) library for dice rolls and supports its syntax.
...
- It is possible to set this behavior by default via the [configuration](../config/display.md#disable-the-comparison).
:::
...
- `value` can be a number, a die, a math expression, or a statistic (just like [custom critical](../sheet/model/critics.md#custom-critics)).
...
It is possible to combine "target count" with exploding dice by **duplicating** the comparison sign, such as `{3d6!>>4}`.
:::roll[ `3d6!>>4` ⟶ `[6!, 5, 3, 1]` = ` [2] `]
:::

This also prevents the exploding dice from exploding on the number after the comparison sign and always explodes on the die's maximum.
...
To comment between each roll, use the `[comment]` syntax. A global comment should use the `#comment` syntax at the end of the dice.[^1]
...
:::danger[Limitations]
- It is not possible to combine shared unique rolls with bulk rolls.
- The first dice cannot be compared. If necessary, it is possible to hide the first die in order to use a comparison in the following formulas.
- Opposition comparison are not supported.
:::
...
[^1]: Shared dice and comments are very sensitive to spaces. Thus, bracketed dice comments must be preceded by a space, as must global comments. The `#` in global comments must be pasted into the comment text, without spaces.