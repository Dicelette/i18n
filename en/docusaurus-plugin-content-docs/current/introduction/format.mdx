...
- **<mark>`[value]`</mark>**:
    - <CL>Usage:</CL> Uses the value in brackets as a "statistic" number that will replace `$` in custom formulas.
    - <CL>Where?</CL> In all user-written dice, macros, and snippets.
    :::example[ `$+2` ]
      - `1d6 + [2]`: `[2]` will be replaced by `2+2`
      - `1d6 > [10]`: `[10]` will be replaced by `10+2`
      - `1d6 + [$dexterity+$constitution]`: `[$dexterity+$constitution]` will be replaced by `$dexterity+$constitution+2`
    :::
