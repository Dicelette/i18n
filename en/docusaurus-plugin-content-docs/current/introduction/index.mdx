...
Moreover, it is possible to use the syntax `dice ; µ+x;µ*y` to roll a single die and apply different formulas to this same die, so as to display the result of each formula. The `µ` symbolizes the result of the die.
...
These formulas do not support comparison with a result (`>=x` for example). The comments between each formula will be merged into a single comment.
