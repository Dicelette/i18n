...
- `:sanitize_short` : Merge between `:short` and `standardize`, except that accents will only be removed for initials (example: `Émile Zola` becomes `EZ` instead of `ÉZ`). Should be used instead of `:short` and `:standardize`.
